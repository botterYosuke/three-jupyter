...
1. **`floating-editor-window`** (`src/components/floating-editor-window.tsx`)
   ...
   - 実行時に自動的に`floating-output-window`を作成（1:1の関係）
...
- **Three.js統合**: `SceneManager`クラスがThree.jsのシーン、カメラ、レンダラーを管理（将来的にCSS2DRendererで3D配置予定）
...
- ウィンドウは現在2Dで表示されているが、将来的にThree.jsの3D空間に配置可能な設計
